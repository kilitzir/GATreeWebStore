...
GA.mutation <-function(response)
{
  ...
  vvar2 = GA.payoff(vvar1,
                    ...
                    unlist(vector1),
                    globalvariables1$parameters$correctionfactor)
  #Somehow (?!), the vvar1 is a dataframe made from vectors containing string and not numeric
  tree = data.frame(lapply(vvar1, as.numeric))
  colnames(tree)<-NULL
  tree <- unlist(tree)
  Ltrees = list(tree = toString(tree),evaluation=toString(vvar2))
